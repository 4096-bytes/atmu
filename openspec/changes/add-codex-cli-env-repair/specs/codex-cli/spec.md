...
`atmu` SHALL 在启动自检阶段通过 `npm list -g`（推荐 `--json`）检测 `@openai/codex-cli` 是否安装。
...
**GIVEN** `npm list -g` 结果中不存在 `@openai/codex-cli`
**WHEN** `atmu` 执行环境检测
**THEN** 检测结果为 `missing`

#### Scenario: 检测已安装

**GIVEN** `npm list -g` 结果中存在 `@openai/codex-cli`
**WHEN** `atmu` 执行环境检测
**THEN** 检测结果为 `ok`

### Requirement: 交互文案语言与 Emoji 约束

`atmu` 在环境检测与修复流程中的所有用户可见交互文案（包括但不限于状态提示、告警信息、菜单项 label）SHALL 全部使用英文。
...
环境检测与修复流程中的状态提示/进度/告警 SHALL 渲染在该“环境状态区域”，而不是渲染在 Banner 区域。
...
### Requirement: 缺失时的 UI 警告与修复入口

当检测结果为 `missing` 时，UI SHALL 在环境状态区域显示告警，并提供默认选中的修复操作。

告警展示 SHOULD 使用 `@inkjs/ui` 的 `Badge`（例如黄色）表达告警级别，而不是在文案中使用 emoji。

`missing` 的告警文案示例：`Codex CLI is not installed`

在 `missing` 修复完成之前，`atmu` SHALL 不开放其他主功能操作（例如配置 API），仅允许用户执行修复相关操作或退出。

#### Scenario: missing 显示未安装警告与一键初始化

**GIVEN** 检测结果为 `missing`
**WHEN** `atmu` 渲染环境状态区域
**THEN** UI 显示 `Codex CLI is not installed`
**AND THEN** UI 使用 `Badge` 表达告警级别
**AND THEN** UI 提供默认选中的修复操作
...
**GIVEN** 检测结果为 `missing`
**WHEN** 用户在主界面确认修复操作
**THEN** `atmu` 执行 `npm install -g @openai/codex-cli`
**AND THEN** 执行成功后自动重新检测
**AND THEN** 检测为 `ok` 后开放主功能操作

### Requirement: 修复过程中的动效进度提示

当 `atmu` 正在执行 `npm install -g ...` 修复操作时，环境状态区域 SHALL 使用 `@inkjs/ui` 的动效组件展示进度/等待状态（例如 `Spinner` 或 `ProgressBar`），而不是仅显示静态文本。

#### Scenario: 修复期间显示动效进度

**GIVEN** 用户已触发修复操作且 `npm install -g ...` 仍在进行中
**WHEN** `atmu` 渲染环境状态区域
**THEN** 环境状态区域显示动效的进度/等待提示

### Requirement: missing 初始化时创建用户 Codex 配置文件

当检测结果为 `missing` 且用户触发修复操作时，`atmu` SHALL 在执行 `npm install -g ...` 之前确保用户目录存在以下路径：
...
#### Scenario: missing 初始化前创建 ~/.codex/config.toml
...
**AND THEN** `atmu` 执行 `npm install -g @openai/codex-cli`

### Requirement: 安装具备超时与重试

安装属于网络操作；`atmu` SHALL 为单次安装尝试设置超时上限以避免假死，并在可恢复失败下自动重试有限次数。

当达到最大重试次数仍失败时，`atmu` SHALL 留在主界面的修复状态并向用户展示失败原因，同时允许用户再次手动触发安装。
