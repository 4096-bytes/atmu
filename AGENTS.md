...
- `src/`: TypeScript/TSX source for the CLI.
  - `src/atmu.tsx`: Pastel entry point that boots the app.
...
- `test/`: AVA tests (UI tests use `ink-testing-library`; E2E tests execute `dist/atmu.js`).
...
- `node dist/atmu.js --help`: run the CLI locally after building.
...
- Add new tests under `test/` (for example `test/ui/*.test.js`, `test/e2e/*.test.js`) and keep imports targeting `dist/` (build first).
